...
const Results: React.FC = () => {
  const [blogs, setBlogs] = useState<Blog[]>([]); //inicialmente son arrays vacíos en los q almacenar lo que se obtrenga de la busqueda
  const [videos, setVideos] = useState<Video[]>([]);
  const [loading, setLoading] = useState(false); //indico si se estan cargando datos, incialmente false, cuando se haga la solicitud cambiua a true
  const [limit] = useState(10); //límite inicial de 10 resultados
  const [offset, setOffset] = useState(0); //incia en cero
  const [hasMore, setHasMore] = useState(true); //verifico si hay más para cargar así puedo hacer scroll infinito
  const [error, setError] = useState<string | null>(null); //almacena cualquier error, incia en null pq no hay
    ...
    try {
      const response = await axios.get("http://localhost:5000/search", {
        params: {
          ...
          query,
          limit,
          offset: reset ? 0 : offset, //si reset es true, el offset es 0 (1er resultado disponible), si no cargo desde el punto actual almacenado en el edo(offset)
        },
      });
  ...
  return (
    <div className="results-container">
      <h2>Results for: {query}</h2> {/*renderizo el prop de busqueda*/}
      {error && <p className="error-text">{error}</p>}{" "}
      {/*renderizo el prop del error*/}
      {blogs.map(
        (
          blog //para cada blog del array blogs devuelvo el id y título q definí en la interfaz y un link en el que se va a renderizar ese item si se hace clic en él
        ) => (
